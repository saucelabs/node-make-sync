...
(function() {
  ...
  _callSync = function() {
    var args, err, f, fWithErr, options, res, wrappedf;
    f = arguments[0], options = arguments[1], args = 3 <= arguments.length ? __slice.call(arguments, 2) : [];
    ...
    wait(res);
    res = res.get();
    switch (options != null ? options.errorType() : void 0) {
      case 'callback':
        err = res[0];
        res = res.slice(1);
        break;
  ...
  makeFuncSync = function(f, options, key) {
    ...
    return function() {
      ...
      switch (callMode) {
        case 'sync':
          return _callSync.apply(this, [f, options].concat(__slice.call(args)));
        case 'async':
